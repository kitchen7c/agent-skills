...
def send_pdf_to_dingtalk(file_path, target_user_id='18669'):
    ...
    try:
        ...
        new_api_res = requests.post(new_api_url, json={
            "appKey": client_id,
...
class ArgusDownloader:
    def __init__(self, headless=True, target_user_id="18669", output_dir=None):
    ...
    def login(self):
        ...
        try:
            ...
            btn = self.page.locator('input[type="submit"][value="Yes"], input[type="button"][value="Yes"], button:has-text("Yes"), input[value="是"], button:has-text("是")').first
    ...
    def get_asia_bitumen_daily(self):
        ...
        with self.page.expect_download() as download_info:
            row = self.page.locator("div, li, tr").filter(has_text="Argus Asia Bitumen Daily").filter(has_text="PDF").last
    ...
    def generate_chinese_report(self, pdf_path, prices=None):
        ...
        api_key = os.environ.get("OPENAI_API_KEY", "sk-1yagyFC2iWQD06Y00Bfnx6VjLpLROnEbmai69JpZOa3AWYVs") 
        base_url = os.environ.get("OPENAI_BASE_URL", "https://agi-prod.chambroad.com/v1") # 请修改为您的gemini兼容代理地址
        model_name = os.environ.get("MODEL_NAME", "gemini-3.1-flash-lite-preview") # 模型名称
        ...
        try:
            ...
            b64_template = base64.b64encode(html_template.encode('utf-8')).decode('utf-8')
...
"""
            ...
            new_pdf_path = os.path.join(os.path.dirname(pdf_path), f"{base_name}_zh.pdf")
    ...
    def get_prices_data(self):
        ...
        prices_section = self.page.locator("div").filter(has=self.page.locator("h2, h3, div", has_text="Prices")).first
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="18669",
        help="指定要推送消息的钉钉用户ID，例如: 18777 (默认: 18669)"
    )
    parser.add_argument(
        '--output-dir',
        type=str,
    ...
    try:
        ...
        if hasattr(downloader, method_name) and callable(getattr(downloader, method_name)):
